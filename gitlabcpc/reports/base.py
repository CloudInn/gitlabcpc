...
from prompts import ReportMilestonesPrompt, ReportProjectsPrompt
import csv
import datetime
import importlib
...
class BaseReport():
    ...
    def get_params(self):
        ("""Get the report's parameters: milestone(s), all projects or group
         projects, etc..""")
        self.only_active_milestones = (
            True if ReportMilestonesPrompt().input == 'yes' else False)
        self.all_projects = (
            True if ReportProjectsPrompt().input == 'all' else False)

    def generate(self):
        ("""Make all the required API calls to return a dictionary containing
         the report results""")
        pass

    def render(self):
        ("""Render the generated results dictionary using the selected """
         """output method""")
        if not self.output:
                ...
                for head in self.data[key]:
                    if 'formatter' in self.data[key][head]:
                        formatter = getattr(
                            ...
                            "format_"+self.data[key][head]['formatter'])
                        writer.writerow([
                            '', head,
                            formatter(self.data[key][head]['value'])])
                    else:
