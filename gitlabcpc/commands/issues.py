...
from prompts import IssuesExportClosedPrompt, IssuesExportLabelPrompt,\
                    IssuesExportMilestonePrompt, IssuesExportProjectPrompt,\
                    LabelNamePrompt, MilestoneNamePrompt,\
                    IssueCloseConfirmationPrompt
import datetime
import sys
import csv
...
class IssuesController(CementBaseController):
    ...
    def close(self):
        ...
        if proceed == 'yes':
            for project in self.app.gl.projects.all(per_page=100):
                closed = 0
                for issue in project.issues.list(milestone=milestone,
                                                 labels=[label_name],
    ...
    def export(self):
        ...
        export_file = open('gitlab-issues-'+now.isoformat()[0:-7]+'.csv', 'w')
        fields = ['id', 'project', 'title', 'description',
                  'labels', 'milestone', 'time spent', 'time estimated',
                  'author', 'assignee', 'due date', 'created',
                  'closed', 'last modified', 'confidential',
        ...
        for project in projects:
            ...
            if only_opened == 'opened':
                pages = project.issues.list(as_list=False,
                                            labels=[label],
                                            milestone=milestone,
                                            state='opened')
            else:
            ...
            while i < pages.total:
                ...
                if assignee:
                    ...
                    assignee = ''
                writer.writerow({'id': issue.iid,
                                 'project': project.name,
                                 'title': issue.attributes['title'],
                                 'description': issue.attributes['description'],
                                 'labels': ';'.join(issue.attributes['labels']),
                                 'milestone': milestone_title,
                                 'time spent': issue.attributes['time_stats']['total_time_spent'],
                                 'time estimated': issue.attributes['time_stats']['time_estimate'],
                                 'author': issue.attributes['author']['username'],
                                 'assignee': assignee,
                                 'due date': issue.attributes['due_date'],
                                 'created': issue.attributes['created_at'],
                                 'closed': issue.attributes['closed_at'],
                                 'last modified': issue.attributes['updated_at'],
                                 'confidential': issue.attributes['confidential'],
                                 'state': issue.state,
                                 'upvotes': issue.attributes['upvotes'],
                                 'downvotes': issue.attributes['upvotes']})
                i = i + 1
            print('issues: %i' % i)
